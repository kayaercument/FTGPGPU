...
int main(int argc, char *argv[]){
	...
	bench_t* d_B_redundant_1 = (bench_t*) malloc(mem_size_B);
	bench_t* d_B_redundant_2 = (bench_t*) malloc(mem_size_B);
	// comparation result
	bool result = false;
	...
	// iniciate B matrix 
		for (int i=0; i<arguments_parameters->size; i++){
	    	for (int j=0; j<arguments_parameters->size; j++){
				...
				d_B_redundant_1[i*arguments_parameters->size+j] = 0;
				d_B_redundant_2[i*arguments_parameters->size+j] = 0;
	    	}
		}
	}
	else
	...
	// copy memory to host
	copy_memory_to_host(relu_bench, d_B, d_B_redundant_1, d_B_redundant_2, size_matrix);
	...
	free(h_B);
	free(d_B);
	free(d_B_redundant_1);
	free(d_B_redundant_2);
return 0;
}
