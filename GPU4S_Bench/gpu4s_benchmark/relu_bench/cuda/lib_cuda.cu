...
__global__ void
relu_kernel(const bench_t *A, bench_t *B1, bench_t *B2, bench_t *B3, const int size, const int m)
{
    unsigned int originalBased = ceil(float(size)/BLOCK_SIZE);
    ...
    if (i < size && j < size){
        ...
        {
            B1[i*size+j] = max(threshold, A[i*size+j]);
        } 
        else if (block_index < (originalBased * 2)) 
        {
            B2[i*size+j] = max(threshold, A[i*size+j]);
        } 
        else 
        {
            B3[i*size+j] = max(threshold, A[i*size+j]);
        }
...
void execute_kernel(GraficObject *device_object, unsigned int n, unsigned int m,unsigned int w){
    ...
    relu_kernel<<<dimGrid, dimBlock>>>(device_object->d_A, device_object->d_B, device_object->d_B_redundant_1, device_object->d_B_redundant_2, n,m);
