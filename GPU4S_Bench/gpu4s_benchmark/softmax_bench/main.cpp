...
int main(int argc, char *argv[]){
	...
	bench_t* h_B_redundant_1 = (bench_t*) malloc(mem_size_B);
	bench_t* h_B_redundant_2 = (bench_t*) malloc(mem_size_B);

	bench_t* sum_h_B = (bench_t*) malloc(sizeof(bench_t));
	bench_t* sum_h_B_redundant_1 = (bench_t*) malloc(sizeof(bench_t));
	bench_t* sum_h_B_redundant_2 = (bench_t*) malloc(sizeof(bench_t));
	// comparation result
	bool result = false;
	...
	// iniciate B matrix 
		for (int i=0; i<arguments_parameters->size; i++){
	    	for (int j=0; j<arguments_parameters->size; j++){
				...
				//h_B_redundant_1[i*arguments_parameters->size+j] = 0;
				//h_B_redundant_2[i*arguments_parameters->size+j] = 0;
	...
	// execute kernel
	execute_kernel(softmax_bench, 
					h_B, h_B_redundant_1, h_B_redundant_2, 
                    sum_h_B, sum_h_B_redundant_1, sum_h_B_redundant_2,
					arguments_parameters->size, arguments_parameters->size, arguments_parameters->size);
	// copy memory to host
	...
	free(A);
	free(h_B);
	free(h_B_redundant_1);
	free(h_B_redundant_2);
	free(d_B);
return 0;
}
