...
#else
__global__ void
wavelet_transform(const bench_t *A, bench_t *B1, bench_t *B2, bench_t *B3, const int n, const bench_t *lowpass_filter,const bench_t *highpass_filter){
    unsigned int size = n;
    ...
    if (i < 3*size){
        ...
        int thread_index = threadIdx.x; // Use threadIdx.x within the block dimension
        ...
        {
            // store the value
		    B1[i+size] = sum_value_high;
        } 
        else if (thread_index < (BLOCK_SIZE * 2)) 
        {
            // store the value
		    B2[i+size] = sum_value_high;
        } 
        else 
        {
            // store the value
		    B3[i+size] = sum_value_high;
        }
    }
}
#endif
...
void execute_kernel(GraficObject *device_object, unsigned int n){
    ...
    dim3 dimGrid(ceil(float(n)/BLOCK_SIZE));
    ...
    wavelet_transform<<<dimGrid, dimBlock>>>(device_object->d_A, device_object->d_B, device_object->d_B_redundant_1, device_object->d_B_redundant_2,  n, device_object->low_filter, device_object->high_filter);
