...
matrix_multiplication_kernel(const bench_t *A,const bench_t *B,  bench_t *C1, bench_t *C2, bench_t *C3, const int n, const int m, const int w)
{
    ...
    unsigned int i = (blockIdx.x % originalBased) * blockDim.x + threadIdx.x;
...
bool device_memory_init(GraficObject *device_object, unsigned int size_a_matrix, unsigned int size_b_matrix, unsigned int size_c_matrix){
   ...
   // Allocate the device input vector A
    ...
    err = cudaMalloc((void **)&device_object->d_C_redundant_1, size_c_matrix * sizeof(bench_t));
    ...
    err = cudaMalloc((void **)&device_object->d_C_redundant_2, size_c_matrix * sizeof(bench_t));
...
void execute_kernel(GraficObject *device_object, unsigned int n, unsigned int m,unsigned int w){
    ...
    matrix_multiplication_kernel<<<dimGrid, dimBlock>>>(device_object->d_A, device_object->d_B, device_object->d_C, device_object->d_C_redundant_1, device_object->d_C_redundant_2            ,n ,m ,w);

    cudaEventRecord(*device_object->stop);
}

void copy_memory_to_host(GraficObject *device_object, bench_t* h_C, bench_t* h_C_redundant_1, bench_t* h_C_redundant_2, int size)
{
    ...
    cudaMemcpy(h_C_redundant_1, device_object->d_C_redundant_1, size * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(h_C_redundant_2, device_object->d_C_redundant_2, size * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaEventRecord(*device_object->stop_memory_copy_host);

    float *majorityVotingResult = findIdenticalArray(h_C, h_C_redundant_1, h_C_redundant_2, size);
...
void clean(GraficObject *device_object){
    ...
    err = cudaFree(device_object->d_C_redundant_1);
    ...
    err = cudaFree(device_object->d_C_redundant_2);
