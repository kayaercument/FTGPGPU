...
int main(int argc, char *argv[])
{
	...
	bench_t* d_C_redundant_1 = (bench_t*) malloc(mem_size_C);
	bench_t* d_C_redundant_2 = (bench_t*) malloc(mem_size_C);
	...
	// iniciate C matrix
		for (int i=0; i<arguments_parameters->size; i++){
	    	for (int j=0; j<arguments_parameters->size; j++){
				...
				d_C_redundant_1[i*arguments_parameters->size+j] = 0;
				d_C_redundant_2[i*arguments_parameters->size+j] = 0;
	    	}
		}
	}
	else
	...
	// copy memory to host
	copy_memory_to_host(matrix_bench, d_C, d_C_redundant_1, d_C_redundant_2, size_matrix);
