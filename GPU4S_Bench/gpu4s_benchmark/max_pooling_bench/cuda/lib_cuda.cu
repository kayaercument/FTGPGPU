...
__global__ void
max_pooling_kernel(const bench_t *A, bench_t *B1, bench_t *B2, bench_t *B3, const int size, const int m, const unsigned int stride,  const unsigned int lateral_stride)
{
    unsigned int originalBased = ceil(((float(size) / stride ))/BLOCK_SIZE);
    unsigned int i = (blockIdx.x % originalBased) * blockDim.x + threadIdx.x;
    unsigned int j = blockIdx.y * blockDim.y + threadIdx.y;

    //unsigned int i = blockIdx.x * BLOCK_SIZE + (threadIdx.x % BLOCK_SIZE);
    // unsigned int j = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (i < size && j < size){
        ...
        {
            B1[i * lateral_stride + j ] = max_value;
        } 
        else if (block_index < (originalBased * 2)) 
        {
            B2[i * lateral_stride + j ] = max_value;
        } 
        else 
        {
            B3[i * lateral_stride + j ] = max_value;
        }
...
void execute_kernel(GraficObject *device_object, unsigned int n, unsigned int m,unsigned int w, unsigned int stride, unsigned int lateral_stride){
    ...
    else
    {
        ...
        dimGrid = dim3(3*ceil(((float(n) / stride ))/BLOCK_SIZE), ceil(((float(m) / stride ))/BLOCK_SIZE));
    }
    
    max_pooling_kernel<<<dimGrid, dimBlock>>>(device_object->d_A, device_object->d_B, device_object->d_B_redundant_1, device_object->d_B_redundant_2,n, m, stride ,lateral_stride);
