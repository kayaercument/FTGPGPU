...
__global__ void
matrix_multiplication_kernel(const bench_t *A,const bench_t *B,  bench_t *C1, bench_t *C2, bench_t *C3, const int n, const int m, const int w)
{
    unsigned int originalBased = ceil(float(n)/BLOCK_SIZE);

    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int j = (blockIdx.y % originalBased) * blockDim.y + threadIdx.y;
...
bool device_memory_init(GraficObject *device_object, unsigned int input_data, unsigned int output_data, unsigned int kernel_1, unsigned int kernel_2, unsigned int stride_1, unsigned int stride_2, unsigned int neurons_dense_1, unsigned int neurons_dense_2){
   // Allocate input
    ...
    // Allocate conv 1 output redundant 1
    err = cudaMalloc((void **)&device_object->conv_1_output_redundant_1, input_data * input_data * sizeof(bench_t));
    ...
    // Allocate conv 1 output redundant 2
    err = cudaMalloc((void **)&device_object->conv_1_output_redundant_2, input_data * input_data * sizeof(bench_t));
    ...
    // Allocate conv 1 output redundant 1
    err = cudaMalloc((void **)&device_object->conv_2_output_redundant_1, size_pooling_1 * size_pooling_1 * sizeof(bench_t));
    ...
    // Allocate conv 1 output redundant 2
    err = cudaMalloc((void **)&device_object->conv_2_output_redundant_2, size_pooling_1 * size_pooling_1 * sizeof(bench_t));
    ...
    // dense layer output 1 redundant_1
    err = cudaMalloc((void **)&device_object->dense_layer_1_output_redundant_1, neurons_dense_1 * sizeof(bench_t));
    ...
    // dense layer output 1 redundant_2
    err = cudaMalloc((void **)&device_object->dense_layer_1_output_redundant_2, neurons_dense_1 * sizeof(bench_t));
    ...
    // dense layer output 2 redundant_1
    err = cudaMalloc((void **)&device_object->dense_layer_2_output_redundant_1, neurons_dense_2 * sizeof(bench_t));
    ...
    // dense layer output 2 redundant_2
    err = cudaMalloc((void **)&device_object->dense_layer_2_output_redundant_2, neurons_dense_2 * sizeof(bench_t));
...
void execute_kernel(GraficObject *device_object, unsigned int input_data, unsigned int output_data, unsigned int kernel_1, unsigned int kernel_2, unsigned int stride_1, unsigned int stride_2, unsigned int neurons_dense_1, unsigned int neurons_dense_2){
    ...
    dim3 dimBlock_Conv, dimGrid_Conv;
    dimBlock_Conv = dim3(BLOCK_SIZE, BLOCK_SIZE);
    dimGrid_Conv = dim3(3*ceil(float(input_data)/BLOCK_SIZE), ceil(float(input_data)/BLOCK_SIZE));

    covolution_kernel<<<dimGrid_Conv, dimBlock_Conv>>>(device_object->input_data, 
                    device_object->conv_1_output, device_object->conv_1_output_redundant_1, device_object->conv_1_output_redundant_2,
                    device_object->kernel_1, input_data, input_data, input_data, kernel_1);
    
    cudaMemcpy(device_object->HOST_conv_1_output, device_object->conv_1_output, input_data * input_data * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_conv_1_output_redundant_1, device_object->conv_1_output_redundant_1, input_data * input_data * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_conv_1_output_redundant_2, device_object->conv_1_output_redundant_2, input_data * input_data * sizeof(bench_t), cudaMemcpyDeviceToHost);

    float *majorityVotingResult = findIdenticalArray(device_object->HOST_conv_1_output, 
                                                     device_object->HOST_conv_1_output_redundant_1, 
                                                     device_object->HOST_conv_1_output_redundant_2, input_data * input_data);
    ...
    {
        device_object->HOST_conv_1_output = majorityVotingResult;
    }

    // Instead of copying from host-to-device again, just point existing pointer on GPU to output? Better way.

    cudaError_t err = cudaMemcpy(device_object->conv_1_output, device_object->HOST_conv_1_output, input_data * input_data * sizeof(bench_t), cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy vector input from host to device (error code %s)!\n", cudaGetErrorString(err));
        return;
    ...
    covolution_kernel<<<dimGrid_Conv, dimBlock_Conv>>>(device_object->pooling_1_output, 
                        device_object->conv_2_output, device_object->conv_2_output_redundant_1, device_object->conv_2_output_redundant_2, 
                        device_object->kernel_2, size_lateral_1, size_lateral_1, size_lateral_1, kernel_2);
    
    cudaMemcpy(device_object->HOST_conv_2_output, device_object->conv_2_output, size_pooling_1 * size_pooling_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_conv_2_output_redundant_1, device_object->conv_2_output_redundant_1, size_pooling_1 * size_pooling_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_conv_2_output_redundant_2, device_object->conv_2_output_redundant_2, size_pooling_1 * size_pooling_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);

    majorityVotingResult = findIdenticalArray(device_object->HOST_conv_2_output, 
                                                     device_object->HOST_conv_2_output_redundant_1, 
                                                     device_object->HOST_conv_2_output_redundant_2, size_pooling_1 * size_pooling_1);
    if (NULL == majorityVotingResult)
    {
        fprintf(stderr, "None of arrays are identical!\n");
    }
    ...
    {
        device_object->HOST_conv_2_output = majorityVotingResult;
    }

    // Instead of copying from host-to-device again, just point existing pointer on GPU to output? Better way.

    err = cudaMemcpy(device_object->conv_2_output, device_object->HOST_conv_2_output, size_pooling_1 * size_pooling_1 * sizeof(bench_t), cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy vector input from host to device (error code %s)!\n", cudaGetErrorString(err));
        return;
    ...
    dimGrid = dim3(3*ceil(float(neurons_dense_1)/dimBlock.x), 1);
    matrix_multiplication_kernel<<<dimGrid, dimBlock>>>(device_object->dense_layer_1_weights, device_object->pooling_2_output,device_object->dense_layer_1_output, device_object->dense_layer_1_output_redundant_1, device_object->dense_layer_1_output_redundant_2,neurons_dense_1, 1, size_lateral_2*size_lateral_2);
    
    cudaMemcpy(device_object->HOST_dense_layer_1_output, device_object->dense_layer_1_output, DENSE_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_dense_layer_1_output_redundant_1, device_object->dense_layer_1_output_redundant_1, DENSE_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_dense_layer_1_output_redundant_2, device_object->dense_layer_1_output_redundant_2, DENSE_1 * sizeof(bench_t), cudaMemcpyDeviceToHost);

    majorityVotingResult = findIdenticalArray(device_object->HOST_dense_layer_1_output, 
                                                     device_object->HOST_dense_layer_1_output_redundant_1, 
                                                     device_object->HOST_dense_layer_1_output_redundant_2, DENSE_1);
    if (NULL == majorityVotingResult)
    {
        fprintf(stderr, "None of arrays are identical!\n");
    }
    ...
    {
        device_object->HOST_dense_layer_1_output = majorityVotingResult;
    }

    // Instead of copying from host-to-device again, just point existing pointer on GPU to output? Better way.

    err = cudaMemcpy(device_object->dense_layer_1_output, device_object->HOST_dense_layer_1_output, sizeof(bench_t) * DENSE_1, cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy vector input from host to device (error code %s)!\n", cudaGetErrorString(err));
        return;
    ...
    // dense layer 2
    dimBlock = dim3(BLOCK_SIZE, 1);
    dimGrid = dim3(ceil(float(neurons_dense_2)/dimBlock.x), 3*1);

    matrix_multiplication_kernel<<<dimGrid, dimBlock>>>(device_object->dense_layer_2_weights, device_object->dense_layer_1_output, device_object->dense_layer_2_output, device_object->dense_layer_2_output_redundant_1, device_object->dense_layer_2_output_redundant_2, neurons_dense_2, 1, neurons_dense_1);
    
    cudaMemcpy(device_object->HOST_dense_layer_2_output, device_object->dense_layer_2_output, DENSE_2 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_dense_layer_2_output_redundant_1, device_object->dense_layer_2_output_redundant_1, DENSE_2 * sizeof(bench_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(device_object->HOST_dense_layer_2_output_redundant_2, device_object->dense_layer_2_output_redundant_2, DENSE_2 * sizeof(bench_t), cudaMemcpyDeviceToHost);

    majorityVotingResult = findIdenticalArray(device_object->HOST_dense_layer_2_output, 
                                                     device_object->HOST_dense_layer_2_output_redundant_1, 
                                                     device_object->HOST_dense_layer_2_output_redundant_2, DENSE_2);
    if (NULL == majorityVotingResult)
    {
        fprintf(stderr, "None of arrays are identical!\n");
    }
    ...
    {
        device_object->HOST_dense_layer_2_output = majorityVotingResult;
    }

    // Instead of copying from host-to-device again, just point existing pointer on GPU to output? Better way.

    err = cudaMemcpy(device_object->dense_layer_2_output, device_object->HOST_dense_layer_2_output, sizeof(bench_t) * DENSE_2, cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy vector input from host to device (error code %s)!\n", cudaGetErrorString(err));
        return;
    ...
    softmax_kernel<<<dimGrid, dimBlock>>>(device_object->dense_layer_2_output, device_object->output_data, device_object->sum_ouput, neurons_dense_2);
    softmax_finish_kernel<<<dimGrid, dimBlock>>>(device_object->output_data, device_object->sum_ouput, neurons_dense_2);
